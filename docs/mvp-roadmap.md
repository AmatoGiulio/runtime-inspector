...
- **`useTunable` / Runtime Value model**: first pass implemented via [RFC 0003](rfcs/0003-runtime-value-model.md): `auto.ts`'s registry is now the shared Runtime Value model behind `useInspector`, `useTunable`, and `// @inspect` alike, with a `dispose()` that releases the claimed name on unmount so remounts don't accumulate suffixes. `useTunable(name, initial, options?)` gives a one-line single-value entry point (`packages/runtime-react-native/src/use-tunable.ts`), landing in the same shared "auto" panel.
